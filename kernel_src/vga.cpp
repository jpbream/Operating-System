...
void VGAGraphicsMode::FillScreen(uint8_t color)
{
    ...
    uint64_t* end = (uint64_t*)(memStart + WIDTH * HEIGHT);
    uint64_t* traveller = (uint64_t*)memStart;
...
{
    uint8_t* scanlineStart = memStart + y * WIDTH + x;
    uint8_t* scanlineEnd = memStart + y * WIDTH +  + x + width;
